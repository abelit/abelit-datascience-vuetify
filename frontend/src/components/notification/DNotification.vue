  ...
  <v-menu offset-y origin="center center">
        ...
        <template v-slot:activator="{ on: tooltip }">
          <v-btn icon v-on="{ ...tooltip, ...menu }">
            <v-badge color="red" overlap left>
              <template v-slot:badge>50</template>
              <v-icon>notifications</v-icon>
            </v-badge>
          </v-btn>
        </template>
        <span>{{$vuetify.lang.t("$vuetify.tooltip.notification")}}</span>
      </v-tooltip>
    </template>
    <v-card max-width="500" class="mx-auto">
      <v-toolbar color="pink" dark>
        ...
        <v-text-field solo-inverted flat hide-details label="Search" append-icon="search" />
